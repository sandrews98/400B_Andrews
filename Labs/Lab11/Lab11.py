...
class M33AnalyticOrbit:
    ...
    def DynamicalFriction(self, Msat, Vc, r, v):
        ...
        Inputs:
            Msat = mass of satellite (M_sun)
            Vc = circular speed at large radii from M31
            r = position vector of M33
            v = velocity vector of M33
        Returns:
            acceleration (decellaration) vector owing to dynamical friction """
        rmag = np.sqrt(r[0]**2 + r[1]**2 + r[2]**2)
        vmag = np.sqrt(v[0]**2 + v[1]**2 + v[2]**2)
        
        #coulomb log ln(Lambda)
        bmax = rmag #current separaion btw M33 and M31
