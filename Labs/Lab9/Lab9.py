...
def EventHorizon(MBH):
    """Function to compute the event horizon of a BH
    Input: MBH is mass of Black hole in Msun
...
def PhotonRing(MBH):
    """Function to compute the lensed photon ring, following Johansen & Psaltis 2010
    input: mass of black hole in Msun
    return: lensed photon radius in AU"""
...
#interpolation makes image smoother
...
circ_Pluto = plt.Circle( [0,0], DPluto.value, fill=False, color='deeppink', linewidth=3,label="Pluto's Orbit")
...
circ_EventHorizon = plt.Circle( [0,0], EH_M87.value, fill=False, color='purple', linewidth=3,label="Event Horizon")
# add the circle to the image
ax.add_patch(circ_EventHorizon)

# Radius at Voyager's current location
## FILL THIS IN 
circ_Voyager = plt.Circle( [0,0], DVoyager.value, fill=False, color='w', linewidth=3,label="Voyager's Orbit")
# add the circle to the image
ax.add_patch(circ_Voyager)

# Lensed Photon Radius
## FILL THIS IN 
circ_PR_M87 = plt.Circle( [0,0], PR_M87.value, fill=False, color='grey', linewidth=3,label='Photon Ring')
# add the circle to the image
ax.add_patch(circ_PR_M87)
